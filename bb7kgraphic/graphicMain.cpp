...
int main (int argc, char* argv[]) {
    ...
    string action;     // check if it is testing mode or normal mode.
    string savedName;     // game save as
    bool test=false;
    bool theme=false;
    if (cin.eof()) return 1;
    if (argc==4) { // loading all information from a file, might go to test mode
        cout << "=======<< Resume >>========" << endl;
    ...
    Xwindow w(theme);
    TextDisplay* d = new TextDisplay(theme, &w);
    Board* b = new Board(theme, &w);
