        ...
        if (canImprove) {
            if (s=="buy") {
                if (level==5) {
                    ...
                    // check "even build" rule for buying a improvement
                    canImprove = true;
                    for (int i=0; i<numNei; i++) {
                        if (neighbour[i]->getLevel()<level) canImprove=false;
                    }
                    if (canImprove) {
                        ...
                        cout << ">> Another property in this Monopoly Block has less improvements than " << name << "." << endl;
                        cout << ">> You cannot buy more improvements for " << name << "!" << endl;
                    }
                }
            } else {
                // check "even build" rule for selling a improvement
                canImprove = true;
                for (int i=0; i<numNei; i++) {
                    if (neighbour[i]->getLevel()>level) canImprove =  false;
                }
                if (canImprove) {
                    ...
                    cout << ">> Another property in this Monopoly Block has more improvements than " << name << "." << endl;
                    cout << ">> You cannot sell any improvement from " << name << "!" << endl;
                }
            }
        } else {
