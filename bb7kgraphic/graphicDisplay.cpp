...
void TextDisplay::addPlayer(string s) {
    if (w!=NULL) w->drawString(513, 555, board[21][54],0);
    for (int i=0; i<8; i++) {
        if (board[21][54][i]==' ') {
    ...
    if (w!=NULL) w->drawString(513, 555, board[21][54],1);
    players[s]=0;
}
...
void TextDisplay::changePosition(string s, int position) {
    ...
    if (w!=NULL) {
        prevPX = w->getPX(prev);
        prevPY = w->getPY(prev);
        w->drawString(prevPX, prevPY, str, w->getColour(prev));
    }
    for (int i=0; i<8; i++) {
        if (str[i]==s[0]) {
    ...
    if (w!=NULL) w->drawString(prevPX, prevPY, board[playerStr[prev][0]][playerStr[prev][1]], 12);
    ...
    if (w!=NULL) {
        px = w->getPX(position);
        py = w->getPY(position);
        w->drawString(px, py, str, w->getColour(position));
    }
    for (int i=0; i<8; i++) {
        if (str[i]==' ') {
    ...
    if (w!=NULL) w->drawString(px, py, board[playerStr[position][0]][playerStr[position][1]], 1);
}

// change the number of I's at position according to new level
void TextDisplay::improve(int level, int position, int ix, int iy) {
    if (w!=NULL) w->drawString(ix, iy, board[improveStr[position].f][improveStr[position].s], w->getColour(position));
    if (level==-1) {
    ...
    if (w!=NULL) w->drawString(ix, iy, board[improveStr[position].f][improveStr[position].s], 1);
}
